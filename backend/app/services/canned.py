...
from app.services.render import analyze_script   # himoyalangan (qwen'da fallback)
from app.services.tts import DEFAULT_VOICE, VOICES, tts

# Semantik moslik OpenAI embeddings'ga asoslangan. Lokal qwen3 gpt.py'da embed_texts
# bo'lmasligi (yoki OPENAI_API_KEY yo'qligi) mumkin → bunda semantik moslik o'chadi,
# faqat aniq/fuzzy moslik ishlaydi (canned baribir funksional qoladi).
try:
    from app.services.gpt import embed_texts as _embed_texts
except ImportError:  # pragma: no cover
    _embed_texts = None


def embed_texts(texts):
    """OpenAI embeddings (mavjud bo'lsa) yoki None (semantik moslik o'chadi)."""
...
def _run(cid, avatar, voice, mode, text, prompt, fps, meta):
    ...
    try:
        ...
        # embed_texts None qaytarsa (qwen/embeddings yo'q) → faqat fuzzy moslik.
        meta["q_emb"] = embed_texts(meta.get("questions", [])) or []
        if mode == "gpt":

...
from app.services.gpt import ask_gpt, build_system_prompt
from app.services.tts import tts, VOICES, DEFAULT_VOICE

# analyze_script (raqam/sana normalizatsiya + segment/bosh-harakat rejasi) OpenAI'ga
# asoslangan. Lokal qwen3 gpt.py'da bo'lmasligi mumkin → LLM'siz sodda fallback
# (matnni jumlalarga bo'ladi; render baribir ishlaydi, faqat normalizatsiya/motion
# rejasisiz). Mavjud bo'lsa to'liq versiya ishlatiladi.
try:
    from app.services.gpt import analyze_script
except ImportError:  # pragma: no cover
    import re as _re

    def analyze_script(text, language="uz"):
        sents = [s.strip() for s in _re.split(r"(?<=[.!?…])\s+", (text or "").strip()) if s.strip()]
        return {"full_text": text, "segments": [
            {"text": s, "pause_after_ms": 250, "pace": "medium", "head_motion": {"type": None}}
            for s in (sents or [text or ""])]}

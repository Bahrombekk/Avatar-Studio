...
from app.realtime.stt import recognize as stt_recognize
...
async def realtime_ws(ws: WebSocket):
    ...
    # Audio bo'laklarini (webm/opus, brauzer MediaRecorder'dan) yig'amiz va
    # "stop"da REST recognize bilan matnga aylantiramiz. REST yo'li ffmpeg orqali
    # audioni normallashtiradi → tanish ISHONCHLI (streaming OGG framing nozik edi).
    audio_buf = []       # bytearray bo'laklari
    capturing = False
    ...
    try:
        while True:
            ...
            if text is not None:
                if text == "ping":
                    ...
                    if not capturing:
                        continue
                    capturing = False
                    blob = b"".join(audio_buf)
                    audio_buf = []
                    if not blob:
                        await send({"type": "error", "message": "Ovoz kelmadi — qaytadan gapiring"})
                        continue
                    t = time.time()
                    try:
                        # REST recognize ffmpeg bilan normallashtiradi (ishonchli).
                        user_text = await loop.run_in_executor(
                            None, lambda: stt_recognize(blob, language))
                    except Exception as e:  # noqa: BLE001
                        await send({"type": "error", "message": str(e)})
                        continue
                    stt_t = round(time.time() - t, 2)
                    if not user_text:
            ...
            # binar audio bo'lak (webm/opus) → buferga yig'amiz
            audio = msg.get("bytes")
            if audio and capturing:
